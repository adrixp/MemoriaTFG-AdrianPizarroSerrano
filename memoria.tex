...
Por otro lado vamos a separar los datos, que forman los puntos en el espacio tridimiensional de los gráfcos, del código donde se generan las representaciones en sí. Para ello se crea un fichero JSON con los datos y se especificará en el componente la manera de obtener estos datos.
\newline
\newline
Respecto al template HTML mostrado a continuación y al visto en la primera iteración se aprecian varios cambios. Para empezar se ha eliminado la entidad \textit{axis} ya que, como veremos más adelante, ahora cada componente gráfico llamará a dicho componente \textit{axis}. Por tanto, solo es necesario declarar una entidad de A-Frame teniendo como atributo el nombre del componente. 
...
Los tres componentes que se desarrollan son \textit{bubble\_chart} (Para el gráfico de burbujas), \textit{bar\_chart}(Para el gráfico de barras cilíndricas o cúbicas) y \textit{line\_chart}(Para el gráfico de líneas).
...
\begin{itemize}
	...
	En este esquema se puede ver la concordancia del HTML visto anteriormente y el componente. Vemos que se definen las cuatro propiedades especificando el tipo de dato. La propiedad más importante y novedosa en esta iteración es la de \textit{datapoints} la cual se le especifica que sea del tipo \textit{asset} (activo o recurso). Esto quiere decir que vamos a poder pasar al componente una ruta donde tengamos nuestro fichero JSON con los datos del gráfico.
	
	La única diferencia de esquema entre los tres componentes es el atributo \textit{type}. Este atributo se usa únicamente en el componente \textit{bar\_chart} para especificar si queremos un gráfico con barras cilíndricas o cúbicas. En posteriores iteraciones se usará este concepto para unificar los componentes en uno siguiendo los estándares de desarrollo de A-Frame.
	\newline
	\newline
	...
	Seguidamente parseamos el fichero con la función JavaScript JSON.parse. Gracias a ella podemos recorrer cada punto del gráfico con un bucle.
...
Se puede apreciar en el esquema del componente que ahora se puede definir la propiedad color.

\lstinputlisting[language=JavaScript, title= Componente axis-line]{code/iteracion2-axis-line.js}
...
\begin{itemize}
	\item Se añade al fichero \textit{webpack.config.js} las siguientes líneas para procesar CSS y JSON	
	\lstinputlisting[language=JavaScript, title= webpack.config.js CSS y JSON]{code/iteracion2-webpack.js}.
	
	\item Añadimos esta dependencia al fichero \textit{package.json}.	
	...
	\item Código CSS para el menú de opciones
	\lstinputlisting[language=Json, title= CSS menú de opciones]{code/iteracion2-menu.json}
...
Cuando arrancamos el proyecto, nuevamente con el comando \textit{start} definido en el fichero \textit{package.json}, se obtiene como resultado la Figura~\ref{fig:iteracion2-resultado}. Se puede apreciar el menú de opciones donde se tiene la posibilidad de seleccionar un tipo de gráfico y cambiarlo dinámicamente sin necesidad de refrescar la página como vemos en la Figura~\ref{fig:iteracion2-resultadob}. Todo ello gracias a la función \textit{update} de A-Frame.
\clearpage

\begin{figure}
	\centering
	\includegraphics[width=12cm, keepaspectratio]{img/iteracion2-resultado}
	\caption{Iteración 2: Resultado}
	\label{fig:iteracion2-resultado}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=12cm, keepaspectratio]{img/iteracion2-resultadob}
...
Seguimos las recomendaciones de Aframe registry y creamos un nuevo repositorio con las best practise :
...
-Must make sense in the context of a WebVR application. This can change later, but the initial components of theRegistry will be under strict curation.
...
\section{Iteración 4. Poblamiento de funcionalidad para el API. Resolución de incidencias de la comunidad.} 
\label{sec:iteracion_4}

-Gran refactorización de código. Componetizamos todo. Ejes y generación de gráficas separados
