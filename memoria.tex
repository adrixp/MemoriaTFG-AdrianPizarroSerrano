...
\begin{enumerate}
	...
	\item Se podrá colocar una o varias visualizaciones en cualquier parte del espacio tridimensional también conocido como escena. Además, mediante la escucha de eventos podrán ser actualizados todos ellos de manera simultánea y dinámica.
...
\lstinputlisting[language=HTML]{code/resultados-bubble-full.html}

Como vemos en el código, se utiliza únicamente la última versión de la librería de A-Frame y la del componente desarrollado en este proyecto. Se construye una escena de realidad virtual con el elemento \textless a-escene\textgreater y dentro de ella se utiliza el elemento \textless a-light\textgreater para dar luminosidad a la escena y \textless a-camera\textgreater el cual indica el punto de vista inicial del usuario. Por último se añade el elemento \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}. Este elemento es interpretado por el módulo desarrollado en este proyecto y genera un gráfico de tipo burbuja con la representación de los puntos descritos en el fichero \textit{dataPositive.json} en el espacio tridimensional.
...
Como se menciona anteriormente, únicamente hay que cambiar la siguiente línea de código respecto al apartado 4.2.1. Únicamente se añade el elemento \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}. Este elemento es interpretado por el módulo desarrollado en este proyecto y genera un gráfico de barras con la representación de los puntos descritos en el fichero \textit{data.json} en el espacio tridimensional. Además se añade otro atributo dispuesto por la librería el cual es la longitud de los ejes que fijamos al valor 12 mediante la propiedad \textit{axis\_length}.
...
Como se menciona en el apartado 4.2.1, únicamente hay que cambiar la siguiente línea de código para visualizar este gráfico. Se añade el elemento \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}. Este elemento es interpretado por el módulo desarrollado en este proyecto y genera un gráfico barras de tipo cilíndrico con la representación de los puntos descritos en el fichero \textit{data.json} en el espacio tridimensional. Además se añade otro atributo dispuesto por la librería que no permite que los ejes sean negativos \textit{axis\_negative}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=12cm]{img/resultados-pie}
	\caption{Resultados: Gráfico de tarta}
	\label{fig:resultados-tarta}
\end{figure}

Como se menciona anteriormente, únicamente hay que cambiar la siguiente línea de código para visualizar este gráfico. Se añade el elemento \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}. Este elemento es interpretado por el módulo desarrollado en este proyecto y genera un gráfico de tarta. Además se añade otro atributo dispuesto por la librería que determina el radio de la tarta \textit{pie\_radius}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=12cm]{img/resultados-doughnut}
	\caption{Resultados: Gráfico de doughnut}
	\label{fig:resultados-donut}
\end{figure}

Como se menciona anteriormente, únicamente hay que cambiar la siguiente línea de código para visualizar este gráfico. Se añade el elemento \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}.Además se añade un nuevo atributo \textit{pie\_doughnut} respecto al apartado anterior, que determina que sea de tipo doughnut.

\lstinputlisting[language=HTML]{code/resultados-doughnut.html}

\subsection{Totem}

Este elemento es proporcionado por el módulo desarrollado en este proyecto. Su objetivo es cambiar los datos del gráfico de manera dinámica utilizando distintas fuentes de datos. Simplemente con un \textit{click} si estamos en un navegador, o con la interacción \textit{touch} si estamos en un dispositivo de realidad virtual cambiaría nuestro gráfico.
...
Para añadir este elemento únicamente hay que cambiar la siguiente línea de código. Se añade el elemento \textless a-entity\textgreater con los atributos \textit{type},  \textit{entity\_id\_list} y \textit{dataPoints\_list}. Es decir, una lista de identificadores de los gráficos que van a cambiar y un listado de ficheros o variables que contengan los nuevos datos a modificar dinámicamente.
...
Como se menciona en el punto 4.2.1, únicamente hay que añadir las siguientes líneas de código para visualizar los dos gráficos. Se añaden dos elementos \textless a-entity\textgreater con los atributos \textit{type} y \textit{dataPoints}. Además se añaden los atributos \textit{axis\_color}, \textit{axis\_length}, \textit{axis\_grid}, \textit{axis\_negative} y \textit{axis\_position}. Estos atributos permiten modificar el color y la longitud de los ejes, permitir que los ejes sean de tipo rejilla, que sean negativos así como su posición respectivamente.

\lstinputlisting[language=HTML]{code/resultados-axis-param.html}

\subsection{Pop up}

El módulo desarrollado en este proyecto permite el uso de un panel informativo o \textit{pop up} para una mejor comprensión de los datos.

\begin{figure}[H]
	\centering
	\includegraphics[width=11cm]{img/resultados-pop-up}
	\caption{Resultados: Gráfico con Pop-up}
	\label{fig:resultados-pop-up}
\end{figure}

Como se menciona anteriormente, únicamente hay que añadir la siguiente línea de código para visualizar el pop up. Se añade al elemento \textless a-entity\textgreater el atributo \textit{show\_popup\_info} que permite visualizar dicha ventana emergente por cada elemento del gráfico, enriqueciendo así la información mostrada por el gráfico.
...
Como se menciona en el punto 4.2.1, únicamente hay que añadir la siguiente línea de código para visualizar la leyenda. Se añade al elemento \textless a-entity\textgreater los atributos  \textit{show\_legend\_info} que permite visualizar dicha ventana emergente, enriqueciendo así la información mostrada por el gráfico. \textit{Show\_legend\_position} y \textit{show\_legend\_rotation} se utilizan para posicionar la leyenda en el espacio tridimensional deseado por el usuario
...
\begin{figure}[H]
	...
	\caption{Resultados: Dashboard}
	\label{fig:dashboard}
\end{figure}

\lstinputlisting[language=HTML, title= Código HTML de ejemplo para Dashboard]{code/resultados-dashboard.html}
...
Se ha dotado de distintas herramientas para mejorar la lectura, filtrado y comprensión de los datos. Además, se ha facilitado la manera de consumir dichos datos por la librería, ya sea mediante ficheros o variables que contengan un formato JSON o mediante llamadas AJAX.
...
Por último y personalmente considerado como objetivo más importante. Se han aprendido conceptos y nuevas tecnologías en un campo apasionante y en pleno crecimiento. En los siguientes apartados se procede a describir los conocimientos aplicados y aquellos que se han aprendido con este proyecto. Todo ello siguiendo los estándares y las \textit{best practices} propuestas por la comunidad.
...
\begin{enumerate}
  ...
  \item Descubrir y formar parte de la comunidad desarrollo de aplicaciones en realidad virtual.
